...
python inference/infer_multi_command.py  \
    ...
    --model_name_or_path ./PTM/$model_name \
    --inference_model_path ./outputs_LLM-CL/cl/$model_name/lora_$now \
    --inference_batch 1 \
    --max_prompt_len 1024 \
    --max_ans_len 512 \
    --seed 1234 \
    --CL_method lora \
    --inference_output_path ./outputs_LLM-CL/cl/$model_name/lora_$now/predictions

# Collect results:
python inference/collect_results.py --inference_tasks C-STANCE,FOMC,MeetingBank,Py150,ScienceQA,NumGLUE-cm,NumGLUE-ds,20Minuten --data_path ./outputs_LLM-CL/cl/$model_name/lora_$now/predictions
