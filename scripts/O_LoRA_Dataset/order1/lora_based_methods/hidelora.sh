...
deepspeed --include=localhost:$gpu_nodes --master_port 25007 training/main.py  \
    ...
    --print_loss \
    --CL_method Hide_LoRA \
    --output_dir ./outputs_LLM-CL/cl_O_order1/$model_name/Hide_LoRA_$now
...
python inference/infer_multi_command.py  \
    ...
    --model_name_or_path ./PTM/$model_name \
    --inference_model_path ./outputs_LLM-CL/cl_O_order1/$model_name/Hide_LoRA_$now \
    --inference_batch 1 \
    --max_prompt_len 1024 \
    --max_ans_len 512 \
    --seed 1234 \
    --CL_method Hide_LoRA \
    --inference_output_path ./outputs_LLM-CL/cl_O_order1/$model_name/Hide_LoRA_$now/predictions

# Collect results:
python inference/collect_results.py --inference_tasks dbpedia,amazon,yahoo,agnews --data_path ./outputs_LLM-CL/cl_O_order1/$model_name/Hide_LoRA_$now/predictions
