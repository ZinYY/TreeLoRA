...
deepspeed --include=localhost:$gpu_nodes --master_port 25000 training/main.py  \
    ...
    --print_loss \
    --CL_method base \
    --output_dir ./outputs_LLM-CL/cl_O_order2/$model_name/base_$now
...
python inference/infer_multi_command.py  \
    ...
    --model_name_or_path ./PTM/$model_name \
    --inference_model_path ./outputs_LLM-CL/cl_O_order2/$model_name/base_$now \
    --inference_batch 1 \
    --max_prompt_len 1024 \
    --max_ans_len 512 \
    --seed 1234 \
    --CL_method base \
    --inference_output_path ./outputs_LLM-CL/cl_O_order2/$model_name/base_$now/predictions

# Collect results:
python inference/collect_results.py --inference_tasks dbpedia,amazon,agnews,yahoo --data_path ./outputs_LLM-CL/cl_O_order2/$model_name/base_$now/predictions
